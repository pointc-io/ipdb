...
func (c *Conn) Durability() command.Durability {
	return c.durability
}

func (c *Conn) Handler() command.Handler {
	return c.handler
}

func (c *Conn) SetHandler(handler command.Handler) command.Handler {
	prev := c.handler
	c.handler = handler
	return prev
}
